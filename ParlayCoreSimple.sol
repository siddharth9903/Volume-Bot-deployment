...
contract ParlayCoreSimple is Ownable, EIP712 {
  ...
  /// @return tokenAmountBought The amount of tokens bought
  function _buyTokens(
    address tokenAddress,
    address to,
    uint256 etherAmountForSwap
  ) internal returns (uint256 tokenAmountBought) {
  ...
  /// @param tokenAddress The address of the token
  function seedLiquidity(address tokenAddress) public {
    Token storage token = tokens[tokenAddress];
    require(
